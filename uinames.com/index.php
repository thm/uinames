...
---
{
  ...
  "region":"United States"
		...
		<div id="statistics">
			<p>Names Generated Per Day (Starting Yesterday)</p>
			<div id="graph">
				<div class="popup">
				...
				<div class="rows">
					<?php
						...
						$zoom = 200; // zoom (px) clips off traffic spikes
						$rows = 3; // rows in graph
						$visibleField = 0;
						
						$max = max($stats->api->daily);
						
						for ($i = $rows; $i >= 1; $i--) {
							$formula = $max * (1 / $rows) * $i / ($zoom / 75); // 75 = px-height of a visible 100% bar
							echo '<p>' . number_format($formula / 1000, 1) . 'K</p>';
							
							// store max visible bar-height
							if ($i == $rows) $visibleField = $formula;
						}
					
					?>
					<p>0</p>
				</div>
				<div id="bars" <?php echo 'style="height: ' . $zoom . 'px;"'; ?>>
					<?php
						
						for ($i = 1; $i < min(count($stats->web->daily), count($stats->api->daily)); $i++) {
							$dataWeb = $stats->web->daily[$i];
							$dataApi = $stats->api->daily[$i];
							
							if ($dataApi > $visibleField) {
								$inflated = ' inflated';
							} else {
								$inflated = '';
							}
							
							$dataWeb = number_format($dataWeb, 0, ',', '.');
							$dataApi = number_format($dataApi, 0, ',', '.');
							
							$relativeWeb = $stats->web->daily_relative[$i];
							$relativeApi = $stats->api->daily_relative[$i];
						
							echo '<div class="bar' . $inflated . '" data-web="' . $dataWeb . '" data-api="' . $dataApi . '"><span style="height: ' . $relativeApi . '%;"><span style="height: ' . $relativeWeb . '%;"></span></span></div>';
						}
			...
			<div id="graph-legend">
				...
				<span class="inflated" title="Results are likely inflated, either due to an attack on the API, or because of an overly excited user">Likely Inflated</span>
			</div>
		</div>
		<div id="facts">
			<div><?php print $regions; ?><p>Regions Available</p></div>
			<div><?php print number_format($available, 0, ',', '.'); ?><p>Names Available</p></div>
			<div><?php print number_format($stats->web->calculated, 0, ',', '.'); ?><p>Names Generated</p></div>
		</div>
	</div>
